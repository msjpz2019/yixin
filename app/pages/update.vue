<template>
	<view>
		<div class="mui-content">
			<div class="logo"><image src="../static/img/logo.jpg" mode=""></image></div>
			<div id="bottom_progressbar">
				<p class="progressbar_text">{{ text }}</p>
				<!-- 进度条开始 可自行替换 -->
				<view class="progress-box">
					<progress :percent="upgrade" activeColor="red" active stroke-width="8" class="progress" />
				</view>
				<!-- 进度条结束 -->
			</div>
		</div>
	...
	export default {
		data() {
			return {
				...
				downPath: [],
				configData: {},
				AppupdateUrl: 'http://www.ms2018.xyz/update.json',
				AppdownloadUrl:"http://192.168.0.23/H5EF3C469.wgt",
				onlineWgt:''
			};
		},
		async onLoad() {
			// #ifdef APP-PLUS
			let current_version = await this.getWidgetInfo();
			let AppupdateUrl = this.AppupdateUrl;
			console.log(current_version)
			if(!current_version){
				console.log('无法获取本地版本');
				return
			}
			let online_version = await this.$api.get('/test.php',{});
			this.onlineWgt = online_version.data.wgt;
			if(true){
				this.downWgt()
			}
			// #endif
		...
		methods: {
			downWgt(){
				console.log('正在下载资源包');
				let wgtUrl = `http://www.ms2018.xyz/${this.onlineWgt}`;
				let dtask = plus.downloader.createDownload( wgtUrl, {filename:"_doc/update/"}, (d,status)=>{  
					if ( status == 200 ) {   
			...
			detectUpgrade() {
				let dtask1 = plus.downloader.createDownload(this.AppupdateUrl, {retryInterval: 3,timeout: 10},(data, status) => {
			 ...
			 */
			Appupdate(config) {
				let downPath = this.AppdownloadUrl;
				let dtask = plus.downloader.createDownload(downPath, {retryInterval: 3,timeout: 10},(d, status) => {
						this.upgrade = 100;
						if (status == 200) {
							this.text = "正在校验安装包是否完整";
							var path = plus.io.convertLocalFileSystemURL(d.filename);
							if (plus.os.name == "iOS") {path = "file://" + path;}
